...
export default function Dashboard({ user }) {
  ...
  // CHANGED: Group by route_loc instead of driver_name
  const topRoutes = useMemo(() => {
    const m = {};
    rows.forEach((r) => {
      const route = (r.route_loc || 'Unknown').trim() || 'Unknown';
      m[route] = (m[route] || 0) + 1;
    });
        ...
        <>
          ...
          <div className="chart-grid">
            ...
            <div className={isAdmin ? 'card' : 'card span-2'}>
              <h3>Most-flagged routes</h3>
              <div className="chart">
                <ResponsiveContainer width="100%" height="100%">
                  {/* CHANGED: data is now topRoutes and YAxis width increased to 140 for longer route names */}
                  <BarChart data={topRoutes} layout="vertical" margin={{ top: 4, right: 16, left: 8, bottom: 4 }}>
                    ...
                    <YAxis type="category" dataKey="name" width={140} tick={{ fontSize: 12, fill: '#333' }} axisLine={false} tickLine={false} />
                    <Tooltip cursor={{ fill: '#f3f6fb' }} formatter={(v) => [v, 'Observations']} />
